...
function MediaTile({ stream, label, quality, muted = false, spotlighted, onSpotlight }) {
  const ref = useRef(null); const [volume, setVolume] = useState(1); const [volumeOpen, setVolumeOpen] = useState(false);
  useEffect(() => { if (ref.current) { ref.current.srcObject = stream; ref.current.volume = volume; } }, [stream, volume]);
  return <div className={`call-tile ${roomStyles.callTile}${spotlighted ? ` spotlighted ${roomStyles.spotlighted}` : ""}`}><video ref={ref} autoPlay muted={muted} playsInline /><div className={roomStyles.tileIdentity}><span>{label}</span><ConnectionIndicator quality={quality}/></div><button className={`tile-spotlight ${roomStyles.spotlightButton}`} type="button" onClick={onSpotlight} title={spotlighted ? `Stop spotlighting ${label}` : `Spotlight ${label}`} aria-label={spotlighted ? `Stop spotlighting ${label}` : `Spotlight ${label}`}><Maximize2 size={14}/></button>{!muted && <div className={`tile-volume${volumeOpen ? " open" : ""}`}><button type="button" aria-label={`Adjust ${label} volume`} onClick={() => setVolumeOpen((value) => !value)}>{volume === 0 ? <VolumeX size={14}/> : <Volume2 size={14}/>}</button><input aria-label={`${label} volume`} type="range" min="0" max="1" step=".05" value={volume} onChange={(event) => setVolume(Number(event.target.value))}/></div>}</div>;
}
