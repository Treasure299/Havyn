...
export function useRoomCall({ room, socket, user, notify }) {
  ...
  const join = useCallback(async () => {
    ...
    try {
      const [stream, config] = await Promise.all([navigator.mediaDevices.getUserMedia(callConstraints()), loadIceConfig(room?.roomId)]);
      ...
      setMuted(false);
      setCameraOff(false);
      socket?.command("call-join", { muted: false, cameraOff: false });
    } catch (error) {
      configureAudioSession("playback");
      notify(error?.name === "NotAllowedError" ? "Allow camera and microphone access to join the call." : "The call could not start. Check your camera, microphone, and network.");
    }
